...
object PlasmaManualStorage {
    private const val PREF = "plasma_manuals_prefs"
    private const val KEY = "plasma_manuals_v1"

    fun loadAll(context: Context): MutableList<PlasmaManualSection> {
        val sp = context.getSharedPreferences(PREF, Context.MODE_PRIVATE)
        val raw = sp.getString(KEY, null) ?: return mutableListOf()
        return try {
            val arr = JSONArray(raw)
            val list = mutableListOf<PlasmaManualSection>()
            for (i in 0 until arr.length()) {
                val obj = arr.optJSONObject(i) ?: continue
                val name = obj.optString("name", "")
                val itemsArr = obj.optJSONArray("items") ?: JSONArray()
                val items = mutableListOf<PlasmaManualItem>()
                for (j in 0 until itemsArr.length()) {
                    ...
                    items.add(PlasmaManualItem(id = id, title = title, fileUrl = url, description = desc))
                }
                list.add(PlasmaManualSection(name = name, items = items))
            }
            list
        } catch (e: Exception) {
            e.printStackTrace()
    ...
    fun saveAll(context: Context, sections: List<PlasmaManualSection>) {
        val sp = context.getSharedPreferences(PREF, Context.MODE_PRIVATE)
        val arr = JSONArray()
    ...
    fun addSection(context: Context, name: String) {
        ...
        list.add(PlasmaManualSection(name))
    ...
    fun addItem(context: Context, sectionName: String, title: String, fileUrl: String, description: String = "") {
        ...
        val sec = list.firstOrNull { it.name.equals(sectionName, ignoreCase = true) } ?: run {
            val s = PlasmaManualSection(sectionName)
            list.add(s)
            s
        }
        sec.items.add(PlasmaManualItem(title = title, fileUrl = fileUrl, description = description))
        saveAll(context, list)
    }
