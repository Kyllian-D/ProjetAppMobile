...
fun ManualScreen() {
    ...
    val sections = remember { mutableStateListOf<ManualSection>().apply { addAll(ManualStorage.loadAll(ctx)) } }
    ...
    Column(modifier = Modifier
        ...
        Row(modifier = Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween) {
            ...
            Button(onClick = { sections.clear(); sections.addAll(ManualStorage.loadAll(ctx)) }) { Text("Refresh") }
        }
        ...
        if (queryText.isEmpty()) {
                        ...
                        Column(modifier = Modifier.padding(12.dp)) {
                            Row(verticalAlignment = Alignment.CenterVertically) {
                                ...
                                if (SessionManager.isSuperadmin(ctx)) {
                                    ...
                                    TextButton(onClick = { ManualStorage.removeSection(ctx, sec.name); sections.clear(); sections.addAll(ManualStorage.loadAll(ctx)) }) { Text("Delete") }
                                }
                            }
                            Spacer(modifier = Modifier.height(8.dp))
                                ...
                                Card(modifier = Modifier.fillMaxWidth().padding(vertical = 4.dp), colors = CardDefaults.cardColors(containerColor = Color(0xFFF7F7F7))) {
                                    Column(modifier = Modifier
                                        ...
                                        Row(modifier = Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.End) {
                                            if (SessionManager.isSuperadmin(ctx)) {
                                                TextButton(onClick = { ManualStorage.removeItem(ctx, sec.name, it.id); sections.clear(); sections.addAll(ManualStorage.loadAll(ctx)) }) { Text("Delete") }
                                            }
                                        }
                                    }
                                }
            ...
            Column(modifier = Modifier.fillMaxWidth()) {
                ...
                val matched = sections.flatMap { sec: ManualSection ->
        ...
        if (showAddSection) {
            ...
            AlertDialog(onDismissRequest = { showAddSection = false }, confirmButton = {
                Button(onClick = {
                    if (input.isNotBlank()) {
                        ManualStorage.addSection(ctx, input.trim())
                        sections.clear(); sections.addAll(ManualStorage.loadAll(ctx))
                    }
                    ...
                    Button(onClick = {
                        if (pickedPdfUri != null) {
                            ...
                            if (saved) {
                                ...
                                ManualStorage.addItem(ctx, section, "New manual", fileUri, "")
                                sections.clear(); sections.addAll(ManualStorage.loadAll(ctx))
...
private fun sanitizeManualStorage(ctx: Context) {
    try {
        ...
        val cleaned = existing.map { sec ->
            // remove items whose title contains "plasma" (case-insensitive)
            val keptItems = sec.items.filter { it.title.contains("plasma", ignoreCase = true).not() }
            ManualSection(sec.name, keptItems.toMutableList())
        }.filter { sec ->
            // remove any section whose name mentions plasma
            sec.name.contains("plasma", ignoreCase = true).not()
        }
        // Save only if different
        ManualStorage.saveAll(ctx, cleaned)
    } catch (e: Exception) {
        e.printStackTrace()
    }
}
...
private fun migratePlasmaFromManualStorage(ctx: Context) {
    try {
        ...
        var migrated = false
        val remaining = mutableListOf<ManualSection>()
        for (sec in raw) {
            if (sec.name.contains("plasma", ignoreCase = true)) {
                // move whole section
                PlasmaManualStorage.addSection(ctx, sec.name)
                sec.items.forEach { PlasmaManualStorage.addItem(ctx, sec.name, it.title, it.fileUrl, it.description) }
                migrated = true
            } else {
                // move items that contain 'plasma'
                val plasmaItems = sec.items.filter { it.title.contains("plasma", ignoreCase = true) }
                val keepItems = sec.items.filter { it.title.contains("plasma", ignoreCase = true).not() }
                if (plasmaItems.isNotEmpty()) {
                    // ensure target Plasma section exists
                    PlasmaManualStorage.addSection(ctx, sec.name)
                    plasmaItems.forEach { PlasmaManualStorage.addItem(ctx, sec.name, it.title, it.fileUrl, it.description) }
                    migrated = true
                }
                if (keepItems.isNotEmpty()) {
                    remaining.add(ManualSection(sec.name, keepItems.toMutableList()))
                }
            }
        }
        if (migrated) {
