...
// Stockage indépendant pour les manuels techniques de PlasmaTYPHOON
data class PlasmaTechManualItem(val id: Long = System.currentTimeMillis(), val title: String, val fileUrl: String, val description: String = "")
data class PlasmaTechManualSection(val name: String, val items: MutableList<PlasmaTechManualItem> = mutableListOf())

object PlasmaTechManualStorage {
    private const val PREF = "plasma_tech_manuals_prefs"
    private const val KEY = "plasma_tech_manuals_v1"

    fun loadAll(context: Context): MutableList<PlasmaTechManualSection> {
        val sp = context.getSharedPreferences(PREF, Context.MODE_PRIVATE)
        val raw = sp.getString(KEY, null) ?: return mutableListOf()
        return try {
            val arr = JSONArray(raw)
            val list = mutableListOf<PlasmaTechManualSection>()
            for (i in 0 until arr.length()) {
                val obj = arr.optJSONObject(i) ?: continue
                val name = obj.optString("name", "")
                val itemsArr = obj.optJSONArray("items") ?: JSONArray()
                val items = mutableListOf<PlasmaTechManualItem>()
                for (j in 0 until itemsArr.length()) {
                    ...
                    items.add(PlasmaTechManualItem(id = id, title = title, fileUrl = url, description = desc))
                }
                list.add(PlasmaTechManualSection(name = name, items = items))
            }
            list
        } catch (e: Exception) {
            e.printStackTrace()
    ...
    fun saveAll(context: Context, sections: List<PlasmaTechManualSection>) {
        val sp = context.getSharedPreferences(PREF, Context.MODE_PRIVATE)
        val arr = JSONArray()
    ...
    fun addSection(context: Context, name: String) {
        ...
        list.add(PlasmaTechManualSection(name))
    ...
    fun addItem(context: Context, sectionName: String, title: String, fileUrl: String, description: String = "") {
        ...
        val sec = list.firstOrNull { it.name.equals(sectionName, ignoreCase = true) } ?: run {
            val s = PlasmaTechManualSection(sectionName)
            list.add(s)
            s
        }
        sec.items.add(PlasmaTechManualItem(title = title, fileUrl = fileUrl, description = description))
        saveAll(context, list)
    }
