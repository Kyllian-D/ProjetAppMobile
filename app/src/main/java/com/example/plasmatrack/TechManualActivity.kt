...
private fun ensureDefaultTechManuals(ctx: Context) {
    try {
        ...
        val doneKey = "default_tech_manual_added_v1"
        if (prefs.getBoolean(doneKey, false)) return

        val resName = "aquatyphoon_technical_manual_user"
...
@Composable
fun TechManualScreen() {
    val ctx = LocalContext.current
    val sections = remember { mutableStateListOf<TechManualSection>().apply { addAll(TechManualStorage.loadAll(ctx)) } }
    ...
    Column(modifier = Modifier
        ...
        Row(modifier = Modifier.fillMaxWidth().padding(top = 8.dp), verticalAlignment = Alignment.CenterVertically) {
            ...
            Text(text = "Tech Manuals", style = MaterialTheme.typography.titleLarge)
        }
        ...
        Row(modifier = Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween) {
            ...
            Button(onClick = { sections.clear(); sections.addAll(TechManualStorage.loadAll(ctx)) }) { Text("Refresh") }
        }
                        ...
                        Column(modifier = Modifier.padding(12.dp)) {
                            Row(verticalAlignment = Alignment.CenterVertically) {
                                ...
                                if (SessionManager.isSuperadmin(ctx)) {
                                    ...
                                    TextButton(onClick = { TechManualStorage.removeSection(ctx, sec.name); sections.clear(); sections.addAll(TechManualStorage.loadAll(ctx)) }) { Text("Delete") }
                                }
                            }
                            Spacer(modifier = Modifier.height(8.dp))
                                ...
                                Card(modifier = Modifier.fillMaxWidth().padding(vertical = 4.dp), colors = CardDefaults.cardColors(containerColor = Color(0xFFF7F7F7))) {
                                    Column(modifier = Modifier
                                        ...
                                        Row(modifier = Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.End) {
                                            if (SessionManager.isSuperadmin(ctx)) {
                                                TextButton(onClick = { TechManualStorage.removeItem(ctx, sec.name, it.id); sections.clear(); sections.addAll(TechManualStorage.loadAll(ctx)) }) { Text("Delete") }
                                            }
                                        }
                                    }
                                }
            ...
            AlertDialog(onDismissRequest = { showAddSection = false }, confirmButton = {
                Button(onClick = {
                    if (input.isNotBlank()) {
                        ...
                        sections.clear(); sections.addAll(TechManualStorage.loadAll(ctx))
                    }
